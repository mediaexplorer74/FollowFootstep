...
namespace GameManager
{
  public abstract class Personnage : Composant
  {
    ...
    {
      this.nbpas = 0;
      this.sprites.Add((SimpleSprite) new SpriteComposant((Composant) this,
          "ombre", new Vector2((float) collision.Width, (float) collision.Height), 
          new Vector2((float) collision.X, (float) collision.Y), 0, 0.3f));
    ...
    {
      this.nbpas = 0;
      this.sprites.Add((SimpleSprite) new SpriteComposant((Composant) this, "ombre",
          new Vector2((float) collision.Width, (float) collision.Height),
          new Vector2((float) collision.X, (float) collision.Y), 0, 0.3f));
      this.souvenir = new List<Empreinte>();
      this.vitesse = 1f;
      this.modeleEmpreinte = "empreinte";
    ...
    public override void Update(GameTime gametime)
    {
      ...
      if (this.nbpas >= 3)
      {
        ...
        if (Game1.game.Joueur.Etat is DelaiSaut)
        {
            ((DelaiSaut)Game1.game.Joueur.Etat).Waiting();
        }
      }
      this.lastPosition = this.position;
    }
    ...
    {
      this.souvenir.Add(e);
      Game1.game.Components.Add((IGameComponent) e);
    }
