...
namespace monogameJam
{
  public class Joueur : Personnage
  {
    ...
    {
      this.Control();
      if (!Program.game.Init && this.etat is Reapparition)
        this.etat.setReady();
...
label_27:
    ...
    public void Control()
    {
      if (this.peutBouger == 0)
      {
        if ((Keyboard.GetState().GetPressedKeys().Length == 0 
                    ...
                    && !(this.etat is monogameJam.Cinematique))
          this.StopAnimation();
        else
          this.DebutAnimation();
        ...
        if (Keyboard.GetState().IsKeyDown(Keys.W)
                    && Keyboard.GetState().IsKeyDown(Keys.D) && Keyboard.GetState().IsKeyDown(Keys.A) && !this.actionObjet)
        {
          ...
          this.typeSprite = 1;
                    ...
                    && Keyboard.GetState().IsKeyDown(Keys.A) && Keyboard.GetState().IsKeyDown(Keys.D) && !this.actionObjet)
        {
    ...
    public void Sauter()
    {
      if (this.relacherSaut && Keyboard.GetState().IsKeyDown(Keys.Space)
                && !(this.etat is monogameJam.Porter) && !(this.etat is Pousser))
      {
    ...
    public void Porter()
    {
      ...
      {
        KeyboardState state = Keyboard.GetState();
        if (state.IsKeyUp(Keys.LeftShift) && (this.etat is Pousser || this.etat is monogameJam.Porter))
        {
    ...
    {
      if (!(this.etat is monogameJam.Cinematique))
        return;
            if (!this.reachFather)
            {
                ...
                {
                    ((monogameJam.Cinematique)this.etat).sansLesMains();
                }
                else
                {
                    this.StopAnimation();
