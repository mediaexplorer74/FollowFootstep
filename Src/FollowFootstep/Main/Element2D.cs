...
namespace monogameJam
{
  public class Element2D : DrawableGameComponent
  {
    ...
    public void AddSprite(SimpleSprite sprite) => this.sprites.Add(sprite);

    public void RemoveSprite(SimpleSprite sprite) => this.sprites.Remove(sprite);
    ...
    protected override void LoadContent()
    {
      if (Element2D.spriteBatch == null)
        Element2D.spriteBatch = new SpriteBatch(Program.game.GraphicsDevice);
    ...
    private static Texture2D creerTexture(string file)
    {
      ...
      Texture2D texture2D = Program.game.Content.Load<Texture2D>(file);
    ...
    public virtual void Remove()
    {
      ...
      foreach (SimpleSprite sprite in this.sprites)
        Program.game.Components.Remove((IGameComponent) sprite);
    }
  }
}
